...
export class Player {
  ...
  public constructor(
    ...
    this._cryptographer = new Cryptographer(process.env.KEY, process.env.IV);
  ...
  private async _onMergeRequest(id: AgentID, message: MergeRequestMessage) {
    if (this._cryptographer.decrypt(message.secret) !== id.serialize() || this._id > id) {
      return;
    }
    ...
    for (const [parcelId, position] of message.changedPositionParcels) {
      ...
      if (parcel) {
        this._beliefs.freeParcels.set(
          parcelId,
          new Parcel(parcelId, parcel.value, position, parcel.agentID)
        );
      }
    }
  }

  private _onAgentUpdateMessage(id: AgentID, message: AgentUpdateMessage): void {
    if (!this._beliefs.team.has(id)) {
      return;
    }
  ...
  public async informParcelUpdate(message: ParcelUpdateMessage): Promise<void> {
