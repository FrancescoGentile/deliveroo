...
export class MonteCarloPlanner {
    ...
    private setNextIntentions() {
        this._state.availablePositions =
            this._state.environment.getParcelsPositions();
        const intentions = this._state.availablePositions.map((p) =>
            Intention.pickup(p),
        );

        if (
            this._lastIntention !== null &&
            this._lastIntention.type !== IntentionType.PUTDOWN
        ) {
            const deliveryPosition =
                this._state.environment.getClosestDeliveryPosition(
                    this.position,
                );
    ...
    private getBestMoveIntention(
        ...
        const promisingPositions =
            this._state.environment.getPromisingPositions(this.position);
        ...
        for (const [movePosition, value] of promisingPositions) {
            ...
            if (actualPaths.has(intention)) {
                ...
                distanceToMovePosition = this._state.environment.distance(
                    this.position,
                    movePosition,
                );
            }
    ...
    private getTree(
        ...
        for (const [idx, node] of children.entries()) {
            const distance = this._state.environment.distance(
                position,
                node.intention.position,
            );
            const { movementDuration } = Config.getInstance();
            const arrivalTime = startTime.add(
                movementDuration.multiply(distance),
            );
            ...
            res[`child_${idx}`] = {
                ...
                visits: node.visits,
                score:
                    node.utility.getValueByInstant(arrivalTime) / node.visits,
                ...this.getTree(
                    node.children,
                    arrivalTime,
                    node.intention.position,
    ...
    public getBestIntention(
        ...
        if (this._nextMoveIntention !== null) {
            ...
            if (path === null) {
                ...
                this._nextMoveIntention =
                    this.getBestMoveIntention(actualPaths);
                // if all the move intentions are not reachable, we return null
                return this._nextMoveIntention === null
                    ? null
                    : this._nextMoveIntention[0];
            }
