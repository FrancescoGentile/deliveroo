...
export class Player {
    ...
    private _actualPaths: HashMap<Intention, [Position, Direction[] | null]> =
        new HashMap();
    ...
    public constructor(
        ...
        this._environment.onOccupiedPositionsChange(() =>
            this.onOccupiedPositionsChange(),
        );
        ...
        while (true) {
            ...
            if (this._planner.position.equals(intention.position)) {
                ...
                for (const direction of directions) {
                    ...
                    if (success) {
                        this._planner.position =
                            this._planner.position.moveTo(direction);
                        break;
                    }
                }
    ...
    private onOccupiedPositionsChange() {
        ...
        const newActualPaths: HashMap<
            Intention,
            [Position, Direction[] | null]
        > = new HashMap();
        const newBlockedBottlenecks: [HashSet<Position>, Intention][] = [];

        const alreadyAdded: boolean[] = new Array(
            oldBlockedBottlenecks.length,
        ).fill(false);
