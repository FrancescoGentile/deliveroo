...
export class Environment {
    ...
    private static readonly OCCUPIED_POSITIONS_CHANGE_EVENT =
        "occupied-positions-change";
    ...
    public static async new(sensors: Sensors): Promise<Environment> {
        ...
        const map = await buildGrid(size, tiles);
        env._grid = map;
    ...
    private removeParcel(parcel: Parcel, position_only = false) {
        if (!position_only) {
            this._state.freeParcels.delete(parcel.id);
        }

        const parcelsInPosition = this._state.positionToParcels.get(
            parcel.position,
        );
        if (parcelsInPosition !== undefined) {
            const idx = parcelsInPosition.findIndex((p) =>
                p.id.equals(parcel.id),
            );
            if (idx !== -1) {
        ...
        if (this._position !== undefined && this._parcelRadius !== undefined) {
            for (const parcel of this._state.freeParcels.values()) {
                if (
                    ...
                    this._position.manhattanDistance(parcel.position) <=
                        this._parcelRadius
                ) {
            ...
            if (parcel.agentID === null) {
                ...
                if (this._state.freeParcels.has(parcel.id)) {
                    ...
                    if (!oldParcel.position.equals(parcel.position)) {
                        ...
                        const parcelsInPosition =
                            this._state.positionToParcels.get(
                                parcel.position,
                            ) || [];
                        parcelsInPosition.push(parcel);
                        this._state.positionToParcels.set(
                            parcel.position,
                            parcelsInPosition,
                        );
                        isChanged = true;
                    ...
                    this._state.freeParcels.set(parcel.id, parcel);
                    const parcelsInPosition =
                        this._state.positionToParcels.get(parcel.position) ||
                        [];
                    parcelsInPosition.push(parcel);
                    this._state.positionToParcels.set(
                        parcel.position,
    ...
    private onAgentSensing(agents: Agent[]) {
        ...
        const numTiles = this._grid.size.rows * this._grid.size.columns;
    ...
    public getPromisingPositions(
        playerPosition: Position,
    ): [Position, number][] {
        const weights = this._grid.tileWeights;
        const numTiles = this._grid.crossableTiles.length;
        ...
        for (const position of agentsPositions) {
            for (let i = -agentRadius; i <= agentRadius; i += 1) {
                for (let j = -agentRadius; j <= agentRadius; j += 1) {
                    ...
                    if (!pos.isValid(this._grid.size)) {
                        // eslint-disable-next-line no-continue
                        continue;
                    }

                    const idx = this._grid.crossableIndexes.get(pos)!;
        ...
        return values.map((value, idx) => [
            this._grid.crossableTiles[indexes[idx]].position,
            value,
        ]);
    }

    public getTileByPosition(position: Position): Tile {
        const idx = this._grid.crossableIndexes.get(position);
        if (idx === undefined) {
            throw new Error("Position not crossable");
        }

        return this._grid.crossableTiles[idx];
    }

    public distance(start: Position, end: Position): number {
        const startIdx = this._grid.crossableIndexes.get(start);
        const endIdx = this._grid.crossableIndexes.get(end);

        if (startIdx === undefined || endIdx === undefined) {
            throw new Error("Position not crossable");
        }

        return this._grid.distances.get([startIdx, endIdx]);
    }
    ...
    public getNextDirections(start: Position, end: Position): Direction[] {
        ...
        for (const n of start.neigbours(this._grid.size)) {
            try {
                if (
                    this.distance(n, end) === distance - 1 &&
                    !this._state.occupiedPositions.has(n)
                ) {
    ...
    public recomputePath(start: Position, end: Position): Direction[] | null {
        const positions = this._grid.crossableIndexes.copy();
        ...
        while (frontier.size() > 0) {
            ...
            for (const next of current.neigbours(this._grid.size)) {
    ...
    public getClosestDeliveryPosition(position: Position): Position {
        const startIdx = this._grid.crossableIndexes.get(position);
        if (startIdx === undefined) {
            throw new Error("Position not crossable");
        }

        return this._grid.closestDelivery[startIdx];
    }
    ...
    public toPDDL(): PDDLProblem {
        const tiles = this._grid.crossableTiles.map(
            (tile) => `(t_${tile.position.row}_${tile.position.column})`,
        );

        const neigbours = [];
        for (const tile of this._grid.crossableTiles) {
            for (const neighbour of tile.position.neigbours(this._grid.size)) {
                neigbours.push(
                    `(${this.getNextDirections(tile.position, neighbour)} t_${
                        tile.position.row
                    }_${tile.position.column} t_${neighbour.row}_${
                        neighbour.column
                    })`,
                );
            }
